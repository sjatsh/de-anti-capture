...
function parse(filePath) {
  const out = { names: [], rva: {} };
  let b;
  try { b = fs.readFileSync(filePath); } catch { return out; }
  try {
    const e_lfanew = b.readUInt32LE(0x3c);
    if (b.readUInt32LE(e_lfanew) !== 0x00004550) return out;      // 'PE\0\0'
    ...
    const ddOff = opt + (magic === 0x20b ? 112 : 96);             // DataDirectory
    const expRva = b.readUInt32LE(ddOff);                          // [0] = 导出表
...
const cache = new Map();
function resolveSystemDll(dllName) {
  ...
  if (!/\.dll$/i.test(dllName)) { p = path.join(sys, dllName + '.dll'); if (fs.existsSync(p)) return p; }
  return null;
}

function getExports(dllName) {
  if (cache.has(dllName)) return cache.get(dllName);
