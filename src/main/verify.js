...
import { rulesFileFor } from './paths.js';
import { buildSpec } from '../config.js';
...
function writeSingleRule(cfg, pid, r) {
  const spec = r.kind === 'hook' ? buildSpec(r) : '';
  fs.writeFileSync(cfg, '# verify (临时，验证完自动还原)\r\n' + `${pid}|${r.kind}|1|${r.dll || ''}|${r.func || ''}|${spec}|verify\r\n`, 'utf8');
}

const hex = (n) => '0x' + (n >>> 0).toString(16);

// 据注入前后观测判定。pass: true=通过 / false=未生效 / null=无法自动判定
function judge(rule, b, a) {
  switch (rule.kind) {
    case 'idle':
      return { pass: a.idle <= 250, before: `空闲 ${b.idle}ms`, after: `空闲 ${a.idle}ms`, detail: a.idle <= 250 ? '空闲时间被归零' : '空闲未归零（检查注入/规则）' };
    case 'fg':
      if (b.fgself !== 0) return { pass: null, before: `前台自认=${b.fgself}`, after: `前台自认=${a.fgself}`, detail: '基线探针正好在前台，无法判定，重试即可' };
      return { pass: a.fgself === 1, before: `前台自认=${b.fgself}`, after: `前台自认=${a.fgself}`, detail: a.fgself === 1 ? 'GetForegroundWindow 被伪装成自身窗口' : '未伪装成前台' };
    case 'uncapture':
      return { pass: b.aff !== 0 && a.aff === 0, before: `affinity=${hex(b.aff)}`, after: `affinity=${hex(a.aff)}`, detail: b.aff !== 0 && a.aff === 0 ? '截屏保护已被剥离为 NONE' : '未变化（检查注入）' };
    case 'hook': {
      const f = (rule.func || '').toLowerCase();
      if (f === 'getsystemmetrics') {
        const ch = a.sm0 !== b.sm0;
        return { pass: ch ? true : null, before: `SM_CXSCREEN=${b.sm0}`, after: `SM_CXSCREEN=${a.sm0}`, detail: ch ? '入参/返回值已被改' : '值未变（透传或未设 a0/ret；可用命中计数确认已挂）' };
      }
      if (f === 'isdebuggerpresent') {
        const ch = a.dbg !== b.dbg;
        return { pass: ch ? true : null, before: `dbg=${b.dbg}`, after: `dbg=${a.dbg}`, detail: ch ? 'IsDebuggerPresent 返回被改' : '值未变（透传；可用命中计数确认已挂）' };
      }
      return { pass: null, before: '-', after: '-', detail: `探针未调用 ${rule.dll}!${rule.func}，无法自动对比；改用「拦截日志」命中计数/观察确认` };
    }
    default:
...
export async function verifyRule(rule) {
  if (!rule || !rule.kind) return { ok: false, msg: '无效规则' };
  if (rule.kind === 'keepalive')
    return { ok: true, pass: null, before: '-', after: '-', detail: '保活无需注入：开启“保活定时器”后目标窗口会定时收到随机 WM_MOUSEMOVE。' };
  ...
  try {
    ...
    await waitFor(() => pid && samples.length >= 2, 4000);
    ...
    await waitFor(() => samples.length >= base + 3, 4000); // 等几拍注入后的观测
