...
export const stateFile = (): string => path.join(app.getPath('userData'), 'state.json');

// DLL 写出的拦截日志 → %TEMP%/KeepAliveHook.log
export const hookLogFile = (): string =>
  path.join(process.env.TEMP || process.env.TMP || app.getPath('temp'), 'KeepAliveHook.log');

// 规则文件写在 DLL 同目录（DLL 启动时读取它）
export const rulesFileFor = (dllPath: string): string =>
  path.join(path.dirname(dllPath), 'KeepAliveHook.rules.txt');
